...
import HA_Entreprise_ServiceMain from "./human-actions.js";
...
import { df_pipeline_withSavePage_categories, df_map_pipeline_withSavePage, df_pipeline_withSavePage_rules } from "@/routes/scraping-services/class/Config/Pipeline/config_save.js";

const arr_category = [...df_pipeline_withSavePage_categories] as const 
type t_category = typeof arr_category[number]

const arr_fct_name = [...HA_Entreprise_ServiceMain.namesOfPipelineFunction()] as const
type t_fct_name = typeof arr_fct_name[number]


const _lib_pipeline = createJsonFromEntries(arr_fct_name.map((name)=>[name,HA_Entreprise_ServiceMain[name].bind(HA_Entreprise_ServiceMain)])) as  t_getLibPipeline<typeof HA_Entreprise_ServiceMain,typeof arr_fct_name>
