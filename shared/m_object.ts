...
//IVoid
export const getEmptyJson = () : IVoid =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    return {} as IVoid;
}
...
export const jsonAddIfNotExist = < T extends IJson , T2  extends IJson > (json : T , jsonToAdd : T2) : t_jsonAddIfNotExist<T,T2> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function isObject<T>(val:T) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const isEmptyJson = (json : any ) : json is IVoid =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const createJsonAsForEach =<K extends t_indexable_key = string, V = any> (arr_element : { [k in K] : any }[]) :t_createJsonAsForEach<K,V>=>{ /*console.log("DEBUG_ME",getCurrentLine());*/
  ...
  for (const element of arr_element) { /*console.log("DEBUG_ME",getCurrentLine());*/
      for(const key in element){ /*console.log("DEBUG_ME",getCurrentLine());*/
          obj[key] = element[key]
      }
  }
  return obj 
...
export const mergeJsonArr =<K extends t_indexable_key , T extends IJson<K,any> > (arr_element :T[]) : t_mergeJsonArr<K,T> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    const transformedData = {};
    for (const item of arr_element) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const cst_entry = <K extends t_indexable_key , V > (key:K,val:V) :t_Entry <K ,V> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    return [key,val]
}

export const getFristEntryFromJson = <T extends IJson>(json : T) : t_Entry <keyof T , T[keyof T]> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
...
< t_entries extends (readonly (t_Entry<K,V>)[])|nullOrUndefined   , K extends t_indexable_key = t_entries extends nullOrUndefined ? t_indexable_key : t_EntryGetKey<t_entries[number]>, V = t_entries extends nullOrUndefined ?  any:t_EntryGetValue<t_entries[number]> >(entries : t_entries)=>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    let res = getEmptyJson() as any 
    if(entries ) res = entries.reduce((acc,entry:t_Entry<K,V>)=>{ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const applyFctToObjectValues = <F extends t_function , O extends IJson = IJson > (obj : O , fct : F) : IJson<keyof O , ReturnType<F>> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    let res = {} as IJson<keyof O , ReturnType<F>>
    for(const key in obj){ /*console.log("DEBUG_ME",getCurrentLine());*/
        res[key] = fct(obj[key])
    }
    return res
}
...
export const applyFctToObjectKeys = <F extends t_function<K_Out,[K]> , K extends t_indexable_key = string ,K_Out extends   t_indexable_key =string , O extends IJson<K> = IJson<K> > (obj : O , fct : F) : t_applyFctToObjectKeys<F,K,K_Out,O> =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const applyFctToObjectEntries = <RO extends IJson , K extends string = string , V extends any  = any ,   O extends IJson<K,V> = IJson<K,V> > (obj : O , fct_entry : t_function<RO,[t_Entry<K,V>]>) : t_applyFctToObjectEntries<RO,K,V,O>  =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    let res = [] as RO[]
    let key : K 
    for(const _key in obj){ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const getAllKeysOfMethodsOfObject =  <O extends Object > (object :O ) =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
    return Object.getOwnPropertyNames(object).filter(function(property) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function setProp <O extends IJson<K,V>, P extends K , _V extends O[P] , K extends t_indexable_key, V = any   >(obj:O, prop:P, value:_V) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function setPropAddIfExist<O extends IJson<K,V>, P extends K ,_V extends O[P] , K extends t_indexable_key,V >(obj:O, prop:P,  _value :_V, addFct :  <V1 extends O[P] , V2 extends _V>(val:V1, addVal:V2) => _V = <V1 extends O[P] , V2 extends _V>(val:V1, addVal:V2) =>{ /*console.log("DEBUG_ME",getCurrentLine());*/ return {...val, ...addVal} }) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function getPropAndInitIfNotExist<O extends IJson, P extends t_indexable_key , InitValue ={}, V = any  >(obj:O, prop:P, initValue : InitValue = {} as InitValue) { /*console.log("DEBUG_ME",getCurrentLine());*/
    let invalidGet
...
export function getObjectFromPropsArrayAndInitIfNotExist<O extends IJson, ArrP extends readonly t_indexable_key[] ,InitValue ={},InvalidValue = undefined >(obj:O,  _propsArray : ArrP, invalidValue : InvalidValue = undefined, initValue :InitValue = {} as InitValue) { /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function createJsonFromMap <K extends t_indexable_key , V > (_map : Map<K,V>){ /*console.log("DEBUG_ME",getCurrentLine());*/
    return Object.fromEntries(_map) as IJson<K,V>
}

export function createMapFromArrArr<V , ArrArr extends readonly ( t_Entry<K,V>)[],V2 = V,K extends t_indexable_key = t_indexable_key > (arrArr:ArrArr,fct:(...args:t_Entry<K,V>)=>V2 = (...args:t_Entry<K,V>)=>{ /*console.log("DEBUG_ME",getCurrentLine());*/return args[1] as unknown as V2} ){ /*console.log("DEBUG_ME",getCurrentLine());*/
    let map = new Map<K,V2>()
    for(const arr of arrArr){ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export function revertMap<K extends t_indexable_key , V > (map:Map<K,V>){ /*console.log("DEBUG_ME",getCurrentLine());*/
    let res_map = new Map()
    for (const [key, value] of map.entries()) {
        if(!Array.isArray(value)){
            ...
            for(const val of value){ /*console.log("DEBUG_ME",getCurrentLine());*/
...
export const pop_map = <K extends t_indexable_key , V > (map : Map<K,V> , key ?: K ) : V =>{ /*console.log("DEBUG_ME",getCurrentLine());*/
